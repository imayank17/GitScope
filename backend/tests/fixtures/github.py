    ...
    return [
        {
            ...
            "draft": False,
            "pull_request": {"html_url": "https://github.com/test-owner/test-repo/pull/1"}
        },
        {
            "number": 2,
            ...
            "merged_at": None,
            "draft": True,
            "pull_request": {"html_url": "https://github.com/test-owner/test-repo/pull/2"}
        }
    ]
...
def respx_mock_github(
    ...
    """
    with respx.mock(assert_all_called=False, base_url=settings.GITHUB_API_BASE_URL) as mock:
        # Mock Repo metadata
        ...
        # Mock Contributors
        mock.get(re.compile(r"/repos/[^/]+/[^/]+/contributors(\?.*)?$"), name="get_contributors").mock(
            return_value=Response(200, json=github_contributors_data, headers={"Link": ""})
        )
        
        # Mock Commits
        mock.get(re.compile(r"/repos/[^/]+/[^/]+/commits(\?.*)?$"), name="get_commits").mock(
            return_value=Response(200, json=github_commits_data, headers={"Link": ""})
        )
        
        # Mock Languages
        mock.get(re.compile(r"/repos/[^/]+/[^/]+/languages(\?.*)?$"), name="get_languages").mock(
            return_value=Response(200, json=github_languages_data)
        )
        
        # Mock Pull Requests
        mock.get(re.compile(r"/repos/[^/]+/[^/]+/pulls(\?.*)?$"), name="get_pulls").mock(
            return_value=Response(200, json=github_pulls_data, headers={"Link": ""})
        )
        ...
        mock.get(re.compile(r"/repos/[^/]+/[^/]+/issues(\?.*)?$"), name="get_issues").mock(
            return_value=Response(200, json=all_issues_mock, headers={"Link": ""})
        )
