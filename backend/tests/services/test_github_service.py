...
@pytest.mark.anyio
async def test_get_repository_success(github_service, respx_mock_github, github_repo_data):
...
@pytest.mark.anyio
async def test_get_contributors_success(github_service, respx_mock_github, github_contributors_data):
    ...
    respx_mock_github["get_contributors"].mock(
        return_value=httpx.Response(
            ...
            headers={"Link": '<https://api.github.com/repos/test-owner/test-repo/contributors?page=2>; rel="next", <https://api.github.com/repos/test-owner/test-repo/contributors?page=5>; rel="last"'}
        )
    )
    
    contribs, total_pages = await github_service.get_contributors("test-owner", "test-repo", page=1)
    assert contribs == github_contributors_data
    assert total_pages == 5


@pytest.mark.anyio
async def test_get_commits_success(github_service, respx_mock_github, github_commits_data):
...
@pytest.mark.anyio
async def test_get_languages_success(github_service, respx_mock_github, github_languages_data):
...
@pytest.mark.anyio
async def test_get_pull_requests_success(github_service, respx_mock_github, github_pulls_data):
...
@pytest.mark.anyio
async def test_get_issues_success_filters_pulls(github_service, respx_mock_github, github_issues_data):
