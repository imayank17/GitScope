...
export class PullRequestsPage {
  ...
  constructor(page: Page) {
    ...
    this.prCards = page.locator('div.space-y-3 > div');
  ...
  getPRCard(title: string): Locator {
    return this.page.locator('div.bg-surface-800\\/40', { has: this.page.locator('h3.font-semibold', { text: title }) });
  }

  async verifyPRVisible(title: string): Promise<boolean> {
    const card = this.getPRCard(title);
    try {
